<script lang="ts">
	...
	let {
		fontSize,
		logoScale
	}: ObsidianLogoProps = $props();

	const GRADIENT_UNIQUE_ID = (Math.floor(Math.random() * 10e6)).toString(16);
</script>

<svg
	width="calc({fontSize}*{logoScale})"
	height="calc({fontSize}*{logoScale})"
	viewBox="0 0 512 512"
	fill="none"
...
>
	...
	<g filter="url(#{GRADIENT_UNIQUE_ID}-a)">
		<path
